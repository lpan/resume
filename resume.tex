...
{\normalsize \hfill lawrencefeipan@gmail.com \par}
{\normalsize \hfill \http{lpan.io} $\bigcdot$ \https{github.com/lpan} \par}

\section*{Summary}{}
\newsummary{languages}{Golang, Python, Scala, JavaScript, C/C++}
\quarterspace
\newsummary{interests}{Distributed systems, Real-time event processing, System programming}
\quarterspace
\newsummary{technologies}{Kubernetes, Linux Kernel, Spark, Postgres, Kafka, Consul, gRPC}
...
\begin{itemize}[noitemsep,leftmargin=20pt,label=\raisebox{0.25ex}{\tiny$\bullet$},topsep=5pt]
  \small
  \item Built Spinnaker pipelines to allow phased rollout for ingress-related
    infrastructure changes
  \item Extended the existing deployment system to support pre-deploy smoke tests
  \item Contributed to the effort of instrumenting distributed tracing, and
    implementing Istio service mesh inside PlanGrid's Kubernetes cluster
\end{itemize}
...
\begin{itemize}[noitemsep,leftmargin=20pt,label=\raisebox{0.25ex}{\tiny$\bullet$},topsep=5pt]
  ...
  \item Worked on the metrics submission pipeline that streams \textbf{millions} of data
    points per second
  \item Owned the time-series creation service written in Golang by ensuring it
    met its SLOs
  \item Spearheaded the migration of the service's IPC layer to gRPC, details in
